...
export default function EditQuestionPage() {
    ...
    try {
      ...
      switch (question.type) {
        case 'multiple-choice':
          setMultipleChoiceData(question.data);
          break;
        case 'true-false':
          setTrueFalseData(question.data);
          break;
        case 'fill-in-blank':
          setFillInBlankData(question.data);
          break;
        case 'numerical':
          setNumericalData(question.data);
          break;
        case 'matching':
          setMatchingData(question.data);
          break;
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (subject) {
          ...
          await fetchTopicsForSubject(subject._id.toString());
        }
      }
    } catch (err) {
      console.error('Error fetching subject by name:', err);
    }
  };
    ...
    try {
      ...
      const response = await fetch(`/api/admin/questions/${params.id}`, {
        ...
        body: JSON.stringify({
          ...
          points: questionData.xpReward, // API expects 'points' field
      ...
      router.push(`/admin/questions/${params.id}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-4">
        <Button variant="outline" size="sm" asChild>
          <Link href={`/admin/questions/${params.id}`}>
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to Question
          </Link>
        </Button>
        <div>
          ...
          <p className="text-gray-600">{formData.subjectName} → {formData.topicName}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end space-x-4 pt-6 border-t">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push(`/admin/questions/${params.id}`)}
            disabled={isLoading}
          >
            Cancel
          </Button>
